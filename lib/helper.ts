...
export const copyObject = async (canvas: fabric.Canvas, target: fabric.Object) => {
  ...
  let _target: fabric.Object | null;
  return new Promise((resolve) => {
    if (!target) {
      _target = canvas.getActiveObject();
    }
...
export const pasteObject = async (canvas: fabric.Canvas) => {
  // First try to read the system clipboard
  try {
    const { type, result } = (await getSystemClipboard()) as any;

    if (result) {
      if (type === "text") {
        createTextbox({ text: result, canvas });
      } else if (type === "image") {
        createFImage({ src: result, canvas });
      }
      return;
    }
  } catch (err) {
...
export const removeObject = (target: fabric.Object, canvas: fabric.Canvas) => {
  ...
  if (target.type === "activeSelection") {
    _target.getObjects().forEach((obj: any) => {
      canvas.remove(obj);
    });
    canvas.discardActiveObject();
  } else {
...
export const groupSelection = (canvas: fabric.Canvas, target: any) => {
  if (!target) {
